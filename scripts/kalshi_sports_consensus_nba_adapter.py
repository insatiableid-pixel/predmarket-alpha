#!/usr/bin/env python3
"""Wrap NBA sharp rows into the strict sports consensus manifest."""
...
from predmarket.sports_consensus_nba_adapter import (  # noqa: E402
    DEFAULT_ALLOWED_BOOKS,
    build_nba_consensus_adapter,
    render_nba_consensus_adapter_markdown,
)
...
DEFAULT_OUT_DIR = MACRO_DIR / "kalshi-sports-consensus-nba-adapter-latest"
...
DEFAULT_NBA_KALSHI_JSON = MACRO_DIR / "latest-kalshi-universe-scan.json"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--nba-kalshi-json", type=Path, default=DEFAULT_NBA_KALSHI_JSON)
    parser.add_argument("--nba-odds-json", type=Path, default=None)
    parser.add_argument("--nba-odds-meta-json", type=Path, default=None)
    parser.add_argument("--out-dir", type=Path, default=DEFAULT_OUT_DIR)
    parser.add_argument("--run-id", default="kalshi-sports-consensus-nba-adapter-latest")
...
def run_nba_consensus_adapter(
    *,
    ...
    nba_kalshi_json: Path = DEFAULT_NBA_KALSHI_JSON,
    nba_odds_json: Path | None = None,
    nba_odds_meta_json: Path | None = None,
    ...
    nba_kalshi = _read_json_object(nba_kalshi_json) if nba_kalshi_json.is_file() else base_kalshi
    odds_path = nba_odds_json or _latest_nba_odds_json(raw_output_dir)
    meta_path = nba_odds_meta_json or _meta_path_for(odds_path)
    if capture_current:
    ...
    reference, combined_kalshi, report = build_nba_consensus_adapter(
        ...
        nba_kalshi_payload=nba_kalshi,
        nba_odds_payload=odds_rows,
        nba_odds_meta=odds_meta,
        existing_reference_path=reference_json,
        base_kalshi_path=base_kalshi_json,
        nba_kalshi_path=nba_kalshi_json,
        nba_odds_path=odds_path,
        nba_odds_meta_path=meta_path,
...
def write_outputs(
    ...
    *,
    ...
    json_path = out_dir / "kalshi-sports-consensus-nba-adapter.json"
    md_path = out_dir / "kalshi-sports-consensus-nba-adapter.md"
    report_text = json.dumps(report, indent=2, sort_keys=True, default=str) + "\n"
    markdown = render_nba_consensus_adapter_markdown(report)
    ...
    if _path_is_within(out_dir, MACRO_DIR):
        latest_json = MACRO_DIR / "latest-kalshi-sports-consensus-nba-adapter.json"
        latest_md = MACRO_DIR / "latest-kalshi-sports-consensus-nba-adapter.md"
...
def _latest_nba_odds_json(raw_output_dir: Path) -> Path | None:
    matches = sorted(raw_output_dir.glob("basketball_nba_current_*.json"))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_nba_consensus_adapter(
        reference_json=args.reference_json,
        combined_kalshi_json=args.combined_kalshi_json,
        base_kalshi_json=args.base_kalshi_json,
        nba_kalshi_json=args.nba_kalshi_json,
        nba_odds_json=args.nba_odds_json,
        nba_odds_meta_json=args.nba_odds_meta_json,
