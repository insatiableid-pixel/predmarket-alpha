#!/usr/bin/env python3
"""Archive sports (baseball) proxy feature observations and attach settled Kalshi labels.

This is the sports analog of ``scripts/kalshi_crypto_proxy_observation_loop.py`` and
reuses its ~85%-generic machinery: snapshot ready feature rows, dedupe by
``observation_id``, match observed Kalshi tickers after game end against public
settlement, and emit label rows.

Sports-specific differences from the crypto loop (cosmetic constants + field schema):
- Reads ``latest-kalshi-sports-proxy-feature-packet.json`` feature rows where
  ``feature_status == "sports_proxy_features_ready"`` (the sports ready variant).
...
- Labels come from KALSHI PUBLIC SETTLEMENT (the contract's actual settlement
  ...
  exactly like the crypto loop reads Kalshi settlement. The strength-model
  ``win_probability`` is a MODEL FEATURE and is never used as the label.
- The optional crypto-proxy orderbook-depth enrichment flag is dropped (it is
  crypto-proxy-specific).
- Every observation and label row is research-only: ``usable=false``,
  ``calibrated_probability=null``, ``expected_value_per_contract=null``.
"""
...
SPORTS_FEATURE_READY = "sports_proxy_features_ready"
CSV_FIELDS = [
    "observation_id",
    "contract_ticker",
    "league",
...
def build_sports_proxy_observation_loop(
    *,
    ...
    new_label_rows = [
        row
        for row in computed_label_rows
    ...
    return {
        ...
        "inputs": {
            ...
            if isinstance(feature_packet, Mapping)
            else None,
        ...
        "method": {
            "observation_rule": "Record ready sports proxy feature rows keyed by exact Kalshi game-winner contract ticker and packet time.",
            "label_rule": "Attach outcomes only from public settled Kalshi market payloads matched by exact ticker.",
            "label_source_note": (
                "Labels come from Kalshi public settlement (the contract's actual settlement outcome). "
                "The official game result / league box score is Kalshi's documented settlement source, but the "
                "label of record is what Kalshi settled."
            ),
            "proxy_boundary": "statsapi/ESPN feeds are model features only and never official settlement labels.",
        ...
        "summary": {
            ...
            "league_counts": counts(row.get("league") for row in all_observations),
            "feature_status_counts": counts(row.get("feature_status") for row in all_observations),
            "label_status_counts": counts(
                row.get("label_status") for row in [*all_label_rows, *label_blocked]
            ),
        ...
        "observation_packet": safe_packet(
            generated_utc=generated,
            packet_type="kalshi_sports_proxy_feature_observations",
            rows=new_observations,
            inputs={"feature_packet_path": str(feature_packet_path)},
        ),
        "label_packet": safe_packet(
            generated_utc=generated,
            packet_type="kalshi_sports_proxy_feature_labels",
            rows=new_label_rows,
            inputs={
                "observation_dir": str(observation_dir),
                "settled_snapshot_path": str(settled_snapshot_path),
...
def due_observed_tickers(
    *,
    ...
    for row in rows:
        due_at = timestamp(
            first_present(row, ["expected_expiration_time", "expiration_time", "close_time"])
        )
...
def observation_due_summary(
    ...
    return {
        ...
        "expected_expiration_bucket_counts": dict(
            sorted(bucket_counter.items(), key=lambda item: item[0])[:12]
        ),
    }
...
def feature_observations(
    feature_packet: Mapping[str, Any], *, feature_packet_path: Path
) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(rows):
        if not isinstance(row, Mapping):
            continue
        # VAL-SFL-026: snapshot only ready feature rows (sports ready variant).
        if row.get("feature_status") != SPORTS_FEATURE_READY:
            continue
        ticker = str(row.get("contract_ticker") or "").strip()
        if not ticker:
            continue
        observation = {
            "schema_version": "KalshiSportsProxyFeatureObservationV1",
            "observation_id": observation_id(
                ticker=ticker, decision_time=generated, source_row_index=index
            ),
            ...
            "yes_interpretation": row.get("yes_interpretation"),
            ...
            "timestamp_source": "sports_proxy_feature_packet_generated_utc",
            ...
            "home_strength": optional_float(row.get("home_strength")),
            "away_strength": optional_float(row.get("away_strength")),
            "strength_source": row.get("strength_source"),
            "home_win_probability": probability(row.get("home_win_probability")),
            "win_probability": probability(row.get("win_probability")),
            "predicted_side": row.get("predicted_side"),
            "mlb_platform_model_status": row.get("mlb_platform_model_status"),
            "mlb_platform_model_probability": probability(
                row.get("mlb_platform_model_probability")
            ),
            "mlb_platform_predicted_side": row.get("mlb_platform_predicted_side"),
            "mlb_platform_threshold_delta": optional_float(row.get("mlb_platform_threshold_delta")),
            "mlb_platform_model_id": row.get("mlb_platform_model_id"),
            "mlb_platform_match_key": row.get("mlb_platform_match_key"),
            "mlb_platform_model_artifact": row.get("mlb_platform_model_artifact"),
            "mlb_platform_model_artifact_sha256": row.get("mlb_platform_model_artifact_sha256"),
            "official_settlement_source": row.get("official_settlement_source"),
...
def build_gates(
    *,
    ...
    return [
        gate(
            "sports_proxy_feature_packet_safe",
            "pass" if feature_safe else "blocked",
            "Ready, research-only sports proxy feature packet is required.",
        ),
        gate(
            "new_observations_recorded",
            "pass" if new_observation_count else "warn",
            f"{new_observation_count} new observation row(s) from latest feature packet.",
        ),
        gate(
            "observations_available",
            "pass" if observation_count else "blocked",
            f"{observation_count} total sports proxy observation row(s).",
        ),
        gate(
            "settled_markets_available",
            "pass" if settled_market_count else "warn",
            f"{settled_market_count} public settled market row(s) loaded.",
        ),
        gate(
            "label_rows_available",
            "pass" if label_count else "warn",
            f"{label_count} sports proxy label row(s) emitted.",
        ),
        gate(
            "manual_drop_dirs_outside_repo",
            "pass" if outside_repo(observation_dir) and outside_repo(label_dir) else "blocked",
...
def loop_status(
    *,
    ...
    if not feature_safe:
        return "sports_proxy_observation_loop_blocked_missing_feature_packet"
    if label_count:
        return "sports_proxy_observation_loop_label_rows_ready"
    if total_observation_count:
        return "sports_proxy_observation_loop_ready_waiting_settlement"
    if new_observation_count:
        return "sports_proxy_observation_loop_observations_recorded_waiting_settlement"
    return "sports_proxy_observation_loop_blocked_no_observations"


def next_action(status: str) -> dict[str, str]:
    if status == "sports_proxy_observation_loop_label_rows_ready":
        return {
    ...
    if status == "sports_proxy_observation_loop_ready_waiting_settlement":
        return {
            ...
            "stop_condition": "Stop before using proxy feeds as labels, model probabilities, EV, sizing, or execution evidence.",
        }
    return {
        "name": "kalshi_sports_proxy_observation_blocker_review",
...
def write_sports_proxy_observation_outputs(
    ...
    *,
    ...
    json_path = out_dir / "kalshi-sports-proxy-observation-loop.json"
    md_path = out_dir / "kalshi-sports-proxy-observation-loop.md"
    csv_path = out_dir / "kalshi-sports-proxy-observation-loop.csv"
    timer_path = out_dir / "kalshi-sports-proxy-observation-loop.timer.example"
    ...
    if observation_rows:
        ...
        obs_path = observation_dir / f"sports_proxy_observations_{stamp}.json"
        obs_latest = observation_dir / "sports_proxy_observations_latest.json"
        obs_text = (
    ...
    if label_rows:
        ...
        label_path = label_dir / f"sports_proxy_labels_{stamp}.json"
        label_latest = label_dir / "sports_proxy_labels_latest.json"
        label_text = (
    ...
    latest_json = MACRO_DIR / "latest-kalshi-sports-proxy-observation-loop.json"
    latest_md = MACRO_DIR / "latest-kalshi-sports-proxy-observation-loop.md"
    latest_csv = MACRO_DIR / "latest-kalshi-sports-proxy-observation-loop.csv"
...
def render_markdown(report: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Leagues: `{summary.get('league_counts')}`",
        "",
        "## Label Source",
        "",
        "Labels come from Kalshi public settlement (the contract's actual settlement outcome).",
        "The official game result / league box score is Kalshi's documented settlement source,",
        "but the label of record is what Kalshi settled. The strength-model win_probability is",
        "a model feature and is never used as the label.",
        "",
        "## Gates",
        "",
        "| Gate | Status | Reason |",
...
# Save as ~/.config/systemd/user/kalshi-sports-proxy-observation-loop.timer after review.

[Unit]
Description=Run research-only Kalshi sports proxy observation loop every 10 minutes
...
# Matching service:
# Save as ~/.config/systemd/user/kalshi-sports-proxy-observation-loop.service after review.
# [Unit]
...
def observation_id(*, ticker: str, decision_time: str, source_row_index: int) -> str:
    ...
    return "sports_obs_" + hashlib.sha256(material).hexdigest()[:20]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.probe_observed_public:
        ...
        if due_tickers:
            settled_snapshot_path = capture_public_observed_markets_snapshot(
                ...
                if settled_snapshot_path.exists()
                else None,
                generated_utc=generated,
            )
            public_calls = True
    report = build_sports_proxy_observation_loop(
        feature_packet_path=args.feature_packet_path,
    ...
    if args.write:
        paths = write_sports_proxy_observation_outputs(
            report,
