#!/usr/bin/env python3
"""Apply deterministic cluster exposure controls to crypto proxy candidates.

This is a research-only gate between capacity/depth evidence and any paper
probability overlay. It reports controlled capacity accounting only; it never
sizes positions, creates usable rows, touches account/order paths, or executes.
"""
...
DEFAULT_CCD_PATH = MACRO_DIR / "latest-kalshi-crypto-proxy-capacity-correlation-decay.json"
DEFAULT_OUT_DIR = MACRO_DIR / "kalshi-crypto-proxy-correlation-cluster-control-latest"
...
def build_crypto_proxy_correlation_cluster_control(
    *,
    ...
    return {
        ...
        "method": {
            "cluster_key": "asset_symbol + contract_family + close_time bucket from the upstream CCD report.",
            "control_rule": (
...
def controlled_capacity_rows(
    ...
    for row in ordered:
        ...
        if cluster_totals.get(key) and sum(cluster_totals.values()) > 0:
            row_copy["controlled_cluster_share"] = json_float(cluster_totals[key] / sum(cluster_totals.values()))
...
def build_summary(
    *,
    ...
    largest_positive_share = largest_positive_cost / total_positive_cost if total_positive_cost > 0 else None
    largest_controlled_share = largest_controlled_cost / total_controlled_cost if total_controlled_cost > 0 else None
    controlled_positive_rows = [row for row in controlled_rows if float(row.get("controlled_depth_cost") or 0.0) > 0]
    return {
        "ccd_status": ccd.get("status"),
        ...
        "asset_counts": counts(row.get("asset_symbol") for row in positive_rows),
        "gate_counts": {},
        "usable_row_count": 0,
    }
...
def report_status(summary_data: Mapping[str, Any], gates: Sequence[Mapping[str, Any]]) -> str:
    if any(item.get("status") == "fail" for item in gates):
        return "crypto_proxy_correlation_cluster_control_failed_safety_gate"
    if gate_status(gates, "ccd_report_ready") != "pass":
        return "crypto_proxy_correlation_cluster_control_blocked_missing_ccd"
    if gate_status(gates, "upstream_capacity_and_decay_pass") != "pass":
        return "crypto_proxy_correlation_cluster_control_blocked_upstream_ccd"
    if int(summary_data.get("positive_depth_row_count") or 0) <= 0:
        return "crypto_proxy_correlation_cluster_control_blocked_no_positive_depth"
    if gate_status(gates, "positive_cluster_breadth") != "pass":
        return "crypto_proxy_correlation_cluster_control_blocked_insufficient_clusters"
    if gate_status(gates, "controlled_cluster_share_limit") != "pass":
        return "crypto_proxy_correlation_cluster_control_blocked_share_limit"
    return "crypto_proxy_correlation_cluster_control_ready_for_paper_overlay"
...
def next_action(status: str) -> dict[str, str]:
    ...
    if status == "crypto_proxy_correlation_cluster_control_blocked_insufficient_clusters":
        return {
            "name": "kalshi_crypto_proxy_cluster_breadth_accumulation",
            "why": "Positive depth is not spread across enough independent clusters to satisfy the configured max share.",
            "stop_condition": "Stop before reducing cluster breadth requirements without an explicit policy review.",
        }
    if status == "crypto_proxy_correlation_cluster_control_blocked_share_limit":
        return {
...
def write_crypto_proxy_correlation_cluster_control(
    ...
    *,
    ...
    json_path = out_dir / "kalshi-crypto-proxy-correlation-cluster-control.json"
    markdown_path = out_dir / "kalshi-crypto-proxy-correlation-cluster-control.md"
    csv_path = out_dir / "kalshi-crypto-proxy-correlation-cluster-control.csv"
    ...
    if should_write_latest:
        ...
        latest_json = target_latest_dir / "latest-kalshi-crypto-proxy-correlation-cluster-control.json"
        latest_md = target_latest_dir / "latest-kalshi-crypto-proxy-correlation-cluster-control.md"
        latest_csv = target_latest_dir / "latest-kalshi-crypto-proxy-correlation-cluster-control.csv"
...
def cluster_rows(cluster_costs: Mapping[str, float], controlled_clusters: Mapping[str, float]) -> list[dict[str, Any]]:
    ...
    return [
        {
            ...
            "controlled_cluster_share": json_float(
                controlled_clusters.get(key, 0.0) / total_controlled if total_controlled > 0 else None
            ),
        }
        for key in keys
    ]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_crypto_proxy_correlation_cluster_control(
        ccd_path=args.ccd_path,
        max_cluster_share=args.max_cluster_share,
        min_positive_clusters=args.min_positive_clusters,
    )
    if args.write:
        paths = write_crypto_proxy_correlation_cluster_control(report, out_dir=args.out_dir)
