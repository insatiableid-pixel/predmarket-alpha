#!/usr/bin/env python3
"""Measure capacity, correlation, and decay gates for crypto proxy candidates.

This is a research-only pre-overlay gate. It may fetch public Kalshi orderbook
snapshots, but it never authenticates, places orders, sizes positions, or marks
contracts usable.
"""
...
DEFAULT_CANDIDATE_SELECTION_POLICY = "cluster_round_robin"
...
def build_crypto_proxy_capacity_correlation_decay(
    *,
    ...
    calibrated_probability = probability(summary(replay).get("conservative_calibrated_side_probability"))
    ...
    return {
        ...
        "inputs": {
            ...
            "max_tickers": max_tickers,
            "depth": depth,
            "candidate_selection_policy": DEFAULT_CANDIDATE_SELECTION_POLICY,
        },
        "method": {
            ...
            "correlation_rule": "Cluster current exposure by asset + contract family + close-time bucket.",
...
def select_current_candidates(
    *,
    ...
    for row in feature_packet.get("feature_rows", []):
        ...
        row_copy = dict(row)
        row_copy["predicted_side"] = side
        row_copy["hours_to_close"] = round(hours_to_close, 6)
        rows.append(row_copy)
    return select_cluster_round_robin(rows, max_tickers=max_tickers)


def select_cluster_round_robin(rows: Sequence[Mapping[str, Any]], *, max_tickers: int) -> list[dict[str, Any]]:
...
def capture_public_orderbooks(
    *,
    ...
    snapshot = {
        ...
        "status": "kalshi_public_orderbook_fetch_ok" if orderbooks else "kalshi_public_orderbook_fetch_empty",
        "research_only": True,
        "execution_enabled": False,
        "market_execution": False,
        "account_or_order_paths": False,
    ...
    snapshot_path = raw_orderbook_dir / f"kalshi_crypto_proxy_orderbooks_{stamp}.json"
    latest_path = raw_orderbook_dir / "kalshi_crypto_proxy_orderbooks_latest.json"
...
def load_latest_orderbook_capture(raw_orderbook_dir: Path) -> dict[str, Any]:
    return read_json_or_empty(raw_orderbook_dir / "kalshi_crypto_proxy_orderbooks_latest.json")
...
def capacity_row(
    ...
    *,
    ...
    for index, level in enumerate(levels):
        ...
        if calibrated_probability is not None and break_even is not None and calibrated_probability > break_even:
...
def ask_levels(orderbook: Mapping[str, Any], side: str) -> list[dict[str, float]]:
    book = orderbook.get("orderbook_fp") if isinstance(orderbook.get("orderbook_fp"), Mapping) else {}
    if not book:
...
def build_summary(
    *,
    ...
    positive_rows = [row for row in capacity_rows if float(row.get("positive_depth_contracts") or 0.0) > 0]
    positive_contracts = sum(float(row.get("positive_depth_contracts") or 0.0) for row in capacity_rows)
    ...
    ordered_cluster_costs = dict(sorted(cluster_costs.items(), key=lambda item: (-item[1], item[0])))
    ...
    return {
        ...
        "orderbook_error_count": len(capture.get("errors", [])) if isinstance(capture.get("errors"), list) else 0,
        ...
        "min_decay_labels": min_decay_labels,
        "conservative_calibrated_side_probability": json_float(calibrated_probability),
        "asset_counts": counts(row.get("asset_symbol") for row in selected),
...
def build_gates(summary_data: Mapping[str, Any], *, raw_orderbook_dir: Path) -> list[dict[str, str]]:
    gates = [
        gate(
            "replay_candidate_ready",
            "pass"
            if summary_data.get("replay_status") == "crypto_proxy_research_candidate_replay_blocked_predeployment_gates"
            else "blocked",
        ...
        gate(
            ...
            "pass" if summary_data.get("capacity_status") == "capacity_depth_positive" else "blocked",
            (
        ...
        gate(
            ...
            "pass" if summary_data.get("correlation_status") == "correlation_cluster_within_limit" else "blocked",
            (
    ...
    summary_data["gate_counts"] = counts(item["status"] for item in gates) if isinstance(summary_data, dict) else {}
    return gates
...
def report_status(summary_data: Mapping[str, Any], gates: Sequence[Mapping[str, Any]]) -> str:
    if any(item.get("status") == "fail" for item in gates):
        return "crypto_proxy_capacity_correlation_decay_failed_safety_gate"
    if gate_status(gates, "replay_candidate_ready") != "pass":
        return "crypto_proxy_capacity_correlation_decay_blocked_missing_replay_candidate"
    if gate_status(gates, "current_candidates_present") != "pass":
        return "crypto_proxy_capacity_correlation_decay_blocked_no_current_candidates"
    if (
        gate_status(gates, "public_orderbook_depth_present") != "pass"
        or gate_status(gates, "positive_capacity_depth") != "pass"
    ):
        return "crypto_proxy_capacity_correlation_decay_blocked_capacity_depth"
    if gate_status(gates, "correlation_cluster_limit") != "pass":
        return "crypto_proxy_capacity_correlation_decay_blocked_correlation_concentration"
    if gate_status(gates, "decay_survival") != "pass":
        return "crypto_proxy_capacity_correlation_decay_blocked_decay_survival"
    return "crypto_proxy_capacity_correlation_decay_ready_for_paper_overlay"


def next_action(status: str) -> dict[str, str]:
    if status == "crypto_proxy_capacity_correlation_decay_ready_for_paper_overlay":
        return {
    ...
    if status == "crypto_proxy_capacity_correlation_decay_blocked_correlation_concentration":
        return {
            "name": "kalshi_crypto_proxy_correlation_cluster_control",
            "why": "Current positive-depth candidates are too concentrated in one venue/asset/contract-family bucket.",
            "stop_condition": "Stop before paper overlay until cluster exposure limits are machine-readable and passing.",
        }
    if status == "crypto_proxy_capacity_correlation_decay_blocked_capacity_depth":
        return {
...
def write_crypto_proxy_capacity_correlation_decay(
    ...
    *,
    ...
    json_path = out_dir / "kalshi-crypto-proxy-capacity-correlation-decay.json"
    markdown_path = out_dir / "kalshi-crypto-proxy-capacity-correlation-decay.md"
    csv_path = out_dir / "kalshi-crypto-proxy-capacity-correlation-decay.csv"
    ...
    if should_write_latest:
        ...
        latest_json = target_latest_dir / "latest-kalshi-crypto-proxy-capacity-correlation-decay.json"
        latest_md = target_latest_dir / "latest-kalshi-crypto-proxy-capacity-correlation-decay.md"
        latest_csv = target_latest_dir / "latest-kalshi-crypto-proxy-capacity-correlation-decay.csv"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--min-positive-capacity-contracts", type=float, default=DEFAULT_MIN_POSITIVE_CAPACITY_CONTRACTS
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_crypto_proxy_capacity_correlation_decay(
    ...
    if args.write:
        paths = write_crypto_proxy_capacity_correlation_decay(report, out_dir=args.out_dir)
