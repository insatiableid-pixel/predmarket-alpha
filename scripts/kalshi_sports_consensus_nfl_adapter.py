#!/usr/bin/env python3
"""Wrap NFL sharp rows into the strict sports consensus manifest."""
...
from predmarket.sports_consensus_nfl_adapter import (  # noqa: E402
    DEFAULT_ALLOWED_BOOKS,
    build_nfl_consensus_adapter,
    render_nfl_consensus_adapter_markdown,
)
...
DEFAULT_OUT_DIR = MACRO_DIR / "kalshi-sports-consensus-nfl-adapter-latest"
...
DEFAULT_NFL_KALSHI_JSON = Path(
    "/home/mrwatson/manual_drops/kalshi/kalshi_nfl_game_series_latest.json"
)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--nfl-kalshi-json", type=Path, default=DEFAULT_NFL_KALSHI_JSON)
    parser.add_argument("--nfl-odds-json", type=Path, default=None)
    parser.add_argument("--nfl-odds-meta-json", type=Path, default=None)
    parser.add_argument("--out-dir", type=Path, default=DEFAULT_OUT_DIR)
    parser.add_argument("--run-id", default="kalshi-sports-consensus-nfl-adapter-latest")
...
def run_nfl_consensus_adapter(
    *,
    ...
    nfl_kalshi_json: Path = DEFAULT_NFL_KALSHI_JSON,
    nfl_odds_json: Path | None = None,
    nfl_odds_meta_json: Path | None = None,
    ...
    nfl_kalshi = _read_json_object(nfl_kalshi_json) if nfl_kalshi_json.is_file() else {}
    odds_path = nfl_odds_json or _latest_nfl_odds_json(raw_output_dir)
    meta_path = nfl_odds_meta_json or _meta_path_for(odds_path)
    if capture_current:
    ...
    reference, combined_kalshi, report = build_nfl_consensus_adapter(
        ...
        nfl_kalshi_payload=nfl_kalshi,
        nfl_odds_payload=odds_rows,
        nfl_odds_meta=odds_meta,
        existing_reference_path=reference_json,
        base_kalshi_path=base_kalshi_json,
        nfl_kalshi_path=nfl_kalshi_json,
        nfl_odds_path=odds_path,
        nfl_odds_meta_path=meta_path,
...
def write_outputs(
    ...
    *,
    ...
    json_path = out_dir / "kalshi-sports-consensus-nfl-adapter.json"
    md_path = out_dir / "kalshi-sports-consensus-nfl-adapter.md"
    report_text = json.dumps(report, indent=2, sort_keys=True, default=str) + "\n"
    markdown = render_nfl_consensus_adapter_markdown(report)
    ...
    if _path_is_within(out_dir, MACRO_DIR):
        latest_json = MACRO_DIR / "latest-kalshi-sports-consensus-nfl-adapter.json"
        latest_md = MACRO_DIR / "latest-kalshi-sports-consensus-nfl-adapter.md"
...
def _latest_nfl_odds_json(raw_output_dir: Path) -> Path | None:
    matches = sorted(raw_output_dir.glob("americanfootball_nfl_current_*.json"))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_nfl_consensus_adapter(
        reference_json=args.reference_json,
        combined_kalshi_json=args.combined_kalshi_json,
        base_kalshi_json=args.base_kalshi_json,
        nfl_kalshi_json=args.nfl_kalshi_json,
        nfl_odds_json=args.nfl_odds_json,
        nfl_odds_meta_json=args.nfl_odds_meta_json,
