...
EVENT_RE = re.compile(r"^KXMLBGAME-(?P<year>\d{2})(?P<month>[A-Z]{3})(?P<day>\d{2})(?P<hour>\d{2})(?P<minute>\d{2})(?P<teams>[A-Z]+)$")
...
def build_type2_reference(
    ...
    *,
    odds_raw_path: Optional[Path] = None,
    odds_meta_path: Optional[Path] = None,
...
def run_type2_reference_builder(
    *,
    ...
    reference_json.write_text(json.dumps(reference, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    report_markdown_path.write_text(render_type2_reference_builder_markdown(report), encoding="utf-8")
...
def parse_kalshi_game_start_utc(event_ticker: str) -> Optional[datetime]:
...
def _reference_rows_for_event(
    ...
    *,
    ...
    if not away_abbr or not home_abbr or commence is None:
        return [], [_skip(event, "missing_team_or_commence_time", "Team abbreviation or commence_time is unavailable.")]
    ...
    if event_match is None:
        return [], [_skip(event, "kalshi_event_not_matched", f"No Kalshi event matched {away} at {home}.")]
    event_ticker, kalshi_event, delta = event_match
    market = _select_h2h_market(event, preferred_bookmaker=preferred_bookmaker)
    if market is None:
        return [], [_skip(event, "h2h_market_not_found", f"No two-sided h2h market found for {away} at {home}.")]
...
def _index_kalshi_game_events(kalshi_payload: Mapping[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    rows = kalshi_payload.get("all_scored") or kalshi_payload.get("markets") or kalshi_payload.get("rows") or []
    for row in rows:
        ...
        event = out.setdefault(event_ticker, {"rows": {}, "start_utc": parse_kalshi_game_start_utc(event_ticker)})
...
def _select_kalshi_event(
    ...
    *,
    ...
    for event_ticker, event in kalshi_events.items():
        if not (event_ticker.endswith(f"{away_abbr}{home_abbr}") or event_ticker.endswith(f"{home_abbr}{away_abbr}")):
            continue
        rows = event.get("rows")
        start = event.get("start_utc")
        if not isinstance(rows, Mapping) or away_abbr not in rows or home_abbr not in rows or not isinstance(start, datetime):
            continue
...
def _select_h2h_market(
    ...
    *,
    ...
    if preferred_bookmaker:
        bookmakers.sort(key=lambda row: 0 if isinstance(row, Mapping) and row.get("key") == preferred_bookmaker else 1)
    away = str(event.get("away_team") or "")
    home = str(event.get("home_team") or "")
    for bookmaker in bookmakers:
        ...
        for market in bookmaker.get("markets") or []:
            ...
            outcomes = {
                ...
                if isinstance(outcome, Mapping) and outcome.get("name") and outcome.get("price") is not None
            }
            if away in outcomes and home in outcomes:
                return str(bookmaker.get("key") or "unknown-bookmaker"), {away: int(outcomes[away]), home: int(outcomes[home])}
    return None
...
def _stable_run_id(reference: Mapping[str, Any]) -> str:
    digest = hashlib.sha256(json.dumps(reference, sort_keys=True, default=str).encode()).hexdigest()[:12]
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Build a local Type 2 sportsbook reference from MLB snapshots.")
    parser.add_argument("--odds-raw-json", default="/home/mrwatson/manual_drops/odds_api/baseball_mlb_current_20260620T225933Z.json")
    parser.add_argument("--odds-meta-json", default="/home/mrwatson/manual_drops/odds_api/baseball_mlb_current_20260620T225933Z.meta.json")
    parser.add_argument("--kalshi-json", default="data/kalshi_mlb_game_series_live_current_20260620T230203Z.json")
    parser.add_argument("--reference-json", default="/home/mrwatson/manual_drops/predmarket/type2-sportsbook-reference.json")
    parser.add_argument("--report-dir", default="docs/codex/artifacts/type2-reference-builder-latest")
