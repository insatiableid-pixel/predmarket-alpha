...
class ExecutionManager:
    ...
    def _record_failed_order(
        ...
        *,
        ...
        fee = kalshi_net_fee(price=market_implied, contract_count=1.0, fee_mode="maker")
        net_edge = model_prob - market_implied - fee
    ...
    def calculate_transaction_costs(self, venue: str, quantity: float, price: float) -> float:
        ...
        Delegates to the canonical fee engine (kalshi_net_fee) per contract.
        """
        if venue.lower() == ACTION_VENUE:
            # Per-contract fee from canonical engine, multiplied by contract count
            per_contract_fee = kalshi_net_fee(
                price=price,
                contract_count=1.0,
                fee_mode="maker",
            )
            return per_contract_fee * quantity

        raise ValueError(
            f"Unsupported action venue for transaction costs: {venue}. Kalshi is the only executable venue."
        )
    ...
    async def stage_order(
        ...
        logger.info(
            f"ORDER-STAGED: {venue} | {contract} | {side} {quantity} @ {price} | Hash: {entry_hash[:8]}"
        )
        return {
            "status": "STAGED",
            "audit_hash": entry_hash,
    ...
    async def execute_order(
        ...
        logger.info(
            f"ORDER-ROUTED: {venue} | {contract} | {side} {quantity} @ {price} | Pre-exec Hash: {entry_hash[:8]}"
        )
        ...
        while retries < self.api_retry_limit:
            try:
                ...
                resp = await self.blocking_call_runner(
                    self.kalshi_api.create_order, create_order_request=order_req
                )
                ...
                retries += 1
                jitter = (
                    time.time() % self.retry_jitter_seconds
                    if self.retry_jitter_seconds > 0
                    else 0.0
                )
                sleep_time = backoff + jitter
                logger.warning(
                    f"API-ERROR: Order submission to {venue} failed: {e}. Retrying in {sleep_time:.2f}s (Attempt {retries}/{self.api_retry_limit})."
                )
                if sleep_time > 0:
