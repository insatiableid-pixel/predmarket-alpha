    ...
    *,
    ...
    report = {
        ...
        "inputs": {
            ...
            "kalshi_sha256": _sha256(kalshi_path) if kalshi_path and kalshi_path.is_file() else None,
            "sportsbook_sha256": _sha256(sportsbook_path) if sportsbook_path and sportsbook_path.is_file() else None,
        ...
        "summary": {
            ...
            "missing_ticker_count": sum(1 for result in reference_results if "missing_explicit_kalshi_ticker" in result["blocker_reasons"]),
            "unknown_ticker_count": sum(1 for result in reference_results if "kalshi_ticker_not_found" in result["blocker_reasons"]),
            "invalid_odds_count": sum(1 for result in reference_results if "invalid_sportsbook_reference" in result["blocker_reasons"]),
...
def render_type2_reference_preflight_markdown(report: Mapping[str, Any]) -> str:
    ...
    for gate in report.get("gates", []):
        lines.append(f"- `{gate.get('name', '')}`: `{gate.get('status', '')}` - {'; '.join(gate.get('reasons', []))}")
...
def _validate_reference_row(
    ...
    *,
    ...
    odds_summary: Optional[Dict[str, float]] = None
    try:
    ...
    *,
    ...
    return [
        ...
        _gate(
            ...
            ["Sportsbook reference JSON supplied." if sportsbook_payload is not None else "Sportsbook reference JSON is missing."],
        ),
        _gate(
        ...
        _gate(
            ...
            "blocked" if "missing_explicit_kalshi_ticker" in blocker_reasons else ("pass" if reference_results else "blocked"),
            [
        ...
        _gate(
            ...
            "blocked" if "kalshi_ticker_not_found" in blocker_reasons else ("pass" if reference_results else "blocked"),
            [
        ...
        _gate(
            ...
            "blocked" if "invalid_sportsbook_reference" in blocker_reasons else ("pass" if reference_results else "blocked"),
            [
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run the local Type 2 sportsbook reference intake preflight.")
    ...
    parser.add_argument("--output-dir", default="docs/codex/artifacts/type2-reference-preflight-latest")
