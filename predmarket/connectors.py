...
class KalshiConnector(HTTPConnector):
    ...
    def __init__(
        self, session: Any = None, base_url: str = "https://api.elections.kalshi.com/trade-api/v2"
    ):
        super().__init__(session=session)
        self.base_url = base_url.rstrip("/")
...
class GDELTConnector(HTTPConnector):
    ...
    def __init__(
        self, session: Any = None, base_url: str = "https://api.gdeltproject.org/api/v2/doc/doc"
    ):
        super().__init__(session=session)
...
class FederalRegisterConnector(HTTPConnector):
    ...
    def __init__(
        self, session: Any = None, base_url: str = "https://www.federalregister.gov/api/v1"
    ):
        super().__init__(session=session)
        self.base_url = base_url.rstrip("/")
            ...
            docs.append(
                SourceDocument(
                    source_id=stable_hash(
                        {"source": self.source_name, "id": item.get("document_number")}
                    ),
                    source=self.source_name,
...
def _parse_gdelt_ts(value: str | None) -> float | None:
    if not value:
        return None
    from datetime import datetime
