...
class NLPEventSignalExtractor:
    ...
    def __init__(self, model_name: str = "cross-encoder/ms-marco-MiniLM-L-6-v2"):
        ...
        if TRANSFORMERS_AVAILABLE:
            try:
                ...
                logger.warning(f"Failed to load transformer model {model_name}: {e}. Initializing semantic fallback.")
        else:
    ...
    def get_event_probability(self, headline: str, question: str) -> Tuple[float, float]:
        ...
        # Positive indicators
        if any(w in normalized for w in ["passes", "pass", "approved", "approve", "success", "succeed", "lowers", "lower", "cut", "cuts", "surpassed", "surpass", "rises", "rise"]):
            score += 2.0
        if any(w in normalized for w in ["likely", "will pass", "agreement reached", "supports", "support"]):
            score += 1.0
            
        # Negative indicators
        if any(w in normalized for w in ["fails", "fail", "rejected", "reject", "blocked", "block", "vetoed", "veto", "vetoes", "unlikely", "delay", "delays"]):
            score -= 2.0
        if any(w in normalized for w in ["gridlock", "opposes", "oppose", "no progress", "stalled", "stall"]):
            score -= 1.0
...
class MacroSignalExtractor:
    ...
    def fetch_fred_rate(self, series_id: str) -> float:
        ...
        try:
            ...
            logger.warning(f"Failed to fetch live FRED rate for {series_id}: {e}. Using hardcoded default.")
